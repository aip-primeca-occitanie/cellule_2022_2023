...
#include "control_kuka.h"
#include "recept.h"
#include "control_cellule_kuka.h"
#include "recept_cellule.h"
#include <ros/ros.h>
#include <string>
#include <iostream>
#include <robot_arm/State.h>
#include <robot_arm/Retour_navette.h>
#include <chrono>
#include <signal.h>
#include <stdlib.h>
#include <stdio.h>
...
Control* p_control_staubli;
Control* p_control_kuka;
// permet d'initialiser le pointeur 
void my_handler(int sig)
{
	request_shutdown = 1;
	p_control_staubli->stop(); // initialisation du stop 
	p_control_kuka->stop();
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	Control control_staubli(noeud, "staubli"); // établissement de la com staubli
	p_control_staubli = &control_staubli;
	Control control_kuka(noeud, "kuka");  // création de la com kuka
	...
	std::string s = "/state";
	Recept state_staubli(noeud, "staubli", s); // établissement de la com "état" staubli
	Recept state_kuka(noeud, "kuka", s); // établissement de la com "état" staubli
...
// parametre envoyé au kuka et au staubli
	control_staubli.doTool("open");
	control_staubli.paramSpeed(fast);
	control_kuka.paramSpeed(fastb2);
	control_staubli.moveCartL(init);
	...
	{	
		...
		if (recept_cel.PS3){
			...
			control_staubli.moveCartL(init11);

			control_kuka.moveCartL(init2);

			control_staubli.moveCartL(init22);
			...
			while (state_staubli._isSettled == 0){ros::spinOnce();}
			control_cel.taquet(tab_t2);
