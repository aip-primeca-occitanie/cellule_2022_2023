...
def TrajectoryResultCallback(resultmsg):
	...
	trajectoryErrorCode2 = resultmsg.result.response.error_code.val
#	accel = []
#	vel = []
#	pos = []
#	sec = []
...
# 	fichiertime = open("./src/Kuka_kr6700/kuka_kr6_700sixx_moveit/scripts/time.txt","a")
...
def BuildSequenceRequest(armgroup,handgroup):
	...
	while(iter<len(armgroup_name)):
		...
		if(iter == 0 or iter == 2 or iter == 3 or iter == 4 or iter == 6):
			motionPlanItem_.blend_radius = 0.1
		else:
...
def ControlCallback(pub_kuka):
	...
	if pub_kuka.data == 1:
		...
		armgroup = moveit_commander.MoveGroupCommander("DN2P4", robot_description="/kuka/robot_description", ns="/kuka")
	elif pub_kuka.data == 3:
		armgroup = moveit_commander.MoveGroupCommander("DN3P1", robot_description="/kuka/robot_description", ns="/kuka")
	elif pub_kuka.data == 4:
		armgroup = moveit_commander.MoveGroupCommander("DN3P4", robot_description="/kuka/robot_description", ns="/kuka")
	elif pub_kuka.data == 5:
		armgroup = moveit_commander.MoveGroupCommander("DP1N2", robot_description="/kuka/robot_description", ns="/kuka")
	elif pub_kuka.data == 6:
		armgroup = moveit_commander.MoveGroupCommander("DP4N2", robot_description="/kuka/robot_description", ns="/kuka")
	elif pub_kuka.data == 7:
		armgroup = moveit_commander.MoveGroupCommander("DP1N3", robot_description="/kuka/robot_description", ns="/kuka")
	elif pub_kuka.data == 8:
		armgroup = moveit_commander.MoveGroupCommander("DP4N3", robot_description="/kuka/robot_description", ns="/kuka")
	else :
		rospy.loginfo("Error callback control kuka \n")
		return
...
if __name__ == "__main__":
	...
	planingScene = moveit_commander.PlanningSceneInterface(ns= "/kuka")
	#Création de la navette
	rospy.loginfo("Import de la navette ...")
	name = "Navette"
	scale = (0.1,0.1,0.1)
	filename = "src/Kuka_kr6700/kuka_kr6_700sixx_moveit/meshes/env/Navette.stl"
