...
Recept::Recept(ros::NodeHandle noeud, std::string robot, std::string topic)
{
	...
	{
		sub = noeud.subscribe("/robot_arm/"+robot+"_cart", 1, &Recept::cartCallback, this);
	}
	else if (topic == "/joint")
	{
...
//Pour le topic /state
void Recept::stateCallback(const robot_arm::State::ConstPtr& msg) 
{
...
//Pour le topic /cart
void Recept::cartCallback(const robot_arm::Cart::ConstPtr& msg) 
{
	_x = msg->x;
	_y = msg->y;
	_z = msg->z;
