...
# 	fichierpos = open("./src/Staubli_tx2_60l/staubli_tx2_60l_moveit/scripts/pos.txt","a")
# 	fichiervel = open("./src/Staubli_tx2_60l/staubli_tx2_60l_moveit/scripts/vel.txt","a")
# 	fichieracc = open("./src/Staubli_tx2_60l/staubli_tx2_60l_moveit/scripts/acc.txt","a")
# 	fichiertime = open("./src/Staubli_tx2_60l/staubli_tx2_60l_moveit/scripts/time.txt","a")
...
def ControlCallback(pub_staubli):
	...
	if pub_staubli.data == 1:
		armgroup = moveit_commander.MoveGroupCommander("DN2P1", robot_description="/staubli/robot_description", ns="/staubli")
	elif pub_staubli.data == 2:
		armgroup = moveit_commander.MoveGroupCommander("DN2P4", robot_description="/staubli/robot_description", ns="/staubli")
	elif pub_staubli.data == 3:
		armgroup = moveit_commander.MoveGroupCommander("DN3P1", robot_description="/staubli/robot_description", ns="/staubli")
	elif pub_staubli.data == 4:
		armgroup = moveit_commander.MoveGroupCommander("DN3P4", robot_description="/staubli/robot_description", ns="/staubli")
	elif pub_staubli.data == 5:
		armgroup = moveit_commander.MoveGroupCommander("DP1N2", robot_description="/staubli/robot_description", ns="/staubli")
	elif pub_staubli.data == 6:
		armgroup = moveit_commander.MoveGroupCommander("DP4N2", robot_description="/staubli/robot_description", ns="/staubli")
	elif pub_staubli.data == 7:
		armgroup = moveit_commander.MoveGroupCommander("DP1N3", robot_description="/staubli/robot_description", ns="/staubli")
	elif pub_staubli.data == 8:
		armgroup = moveit_commander.MoveGroupCommander("DP4N3", robot_description="/staubli/robot_description", ns="/staubli")
	else :
		rospy.loginfo("Error callback control staubli \n")
		return
	# on récupère le groupe associé à la pince du robot
	handgroup = moveit_commander.MoveGroupCommander("staubli_hand", robot_description="/staubli/robot_description", ns="/staubli")
	...
	while(True):
		...
		if(finished_before_timeout & (trajectoryErrorCode4 == 1)):
			...
			if(finished_before_timeout & (trajectoryErrorCode4 == 1)):
				...
				mymsgStaubli.FinDeplacerR3 = 1
				rospy.loginfo(mymsgStaubli)
				pub_fintache.publish(mymsgStaubli)
				rospy.sleep(1)
				mymsgStaubli.FinDeplacerR3 = 0
				pub_fintache.publish(mymsgStaubli)
...
if __name__ == "__main__":
	...
	rospy.init_node('execute_trajectory_staubli',anonymous=True)
	mymsgStaubli = FinDeplacerPiece_Msg()
	rospy.Subscriber('/control_robot_staubli',Int32, ControlCallback)
	# Pour ploter les trajectoires du robot
	rospy.Subscriber('/staubli/sequence_move_group/result', MoveGroupSequenceActionResult, TrajectoryResultCallback)
	...
	clientSequence_ = actionlib.SimpleActionClient('/staubli/sequence_move_group', MoveGroupSequenceAction)
	...
	planingScene = moveit_commander.PlanningSceneInterface(ns= "/staubli")
	#Création de la navette
	rospy.loginfo("Import de la navette ...")
	name = "Navette"
	scale = (0.1,0.1,0.1)
	filename = "src/Staubli_tx2_60l/staubli_tx2_60l_moveit/meshes/env/Navette.stl"
