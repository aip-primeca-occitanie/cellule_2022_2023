...
def TrajectoryResultCallback(resultmsg):
	...
	trajectoryErrorCode3 = resultmsg.result.response.error_code.val
	# accel = []
	# vel = []
	# pos = []
	# sec = []
	...
	# fichierpos = open("./src/Yaskawa_hc10/yaskawa1/scripts/pos.txt","a")
	# fichiervel = open("./src/Yaskawa_hc10/yaskawa1/scripts/vel.txt","a")
	# fichieracc = open("./src/Yaskawa_hc10/yaskawa1/scripts/acc.txt","a")
	# fichiertime = open("./src/Yaskawa_hc10/yaskawa1/scripts/time.txt","a")
...
def ControlCallback(pub_yaska1):
	...
	if pub_yaska1.data == 1:
		armgroup = moveit_commander.MoveGroupCommander("DN2P1", robot_description="/yaska1/robot_description", ns="/yaska1")
	elif pub_yaska1.data == 2:
		armgroup = moveit_commander.MoveGroupCommander("DN2P4", robot_description="/yaska1/robot_description", ns="/yaska1")
	elif pub_yaska1.data == 3:
		armgroup = moveit_commander.MoveGroupCommander("DN3P1", robot_description="/yaska1/robot_description", ns="/yaska1")
	elif pub_yaska1.data == 4:
		armgroup = moveit_commander.MoveGroupCommander("DN3P4", robot_description="/yaska1/robot_description", ns="/yaska1")
	elif pub_yaska1.data == 5:
		armgroup = moveit_commander.MoveGroupCommander("DP1N2", robot_description="/yaska1/robot_description", ns="/yaska1")
	elif pub_yaska1.data == 6:
		armgroup = moveit_commander.MoveGroupCommander("DP4N2", robot_description="/yaska1/robot_description", ns="/yaska1")
	elif pub_yaska1.data == 7:
		armgroup = moveit_commander.MoveGroupCommander("DP1N3", robot_description="/yaska1/robot_description", ns="/yaska1")
	elif pub_yaska1.data == 8:
		armgroup = moveit_commander.MoveGroupCommander("DP4N3", robot_description="/yaska1/robot_description", ns="/yaska1")
	else :
		rospy.loginfo("Error callback control yaska1 \n")
		return
		# on récupère le groupe associé à la pince du robot
	handgroup = moveit_commander.MoveGroupCommander("yaskawa1_hand", robot_description="/yaska1/robot_description", ns="/yaska1")
	...
	while(True):
		...
		if(finished_before_timeout & (trajectoryErrorCode3 == 1)):
			...
			if(finished_before_timeout & (trajectoryErrorCode3 == 1)):
				...
				rospy.loginfo("Results: %s" %clientSequence_.action_client.ActionResult)
				#si tout s'est bien passé, on averti coppeliaSim de la fin d'execution du movement du robot
				mymsgyaska1.FinDeplacerR3 = 1
				rospy.loginfo(mymsgyaska1)
				pub_fintache.publish(mymsgyaska1)
				rospy.sleep(1)
				mymsgyaska1.FinDeplacerR4 = 0
				pub_fintache.publish(mymsgyaska1)
...
if __name__ == "__main__":
	...
	rospy.init_node('execute_trajectory_yaska1',anonymous=True)
	mymsgyaska1 = FinDeplacerPiece_Msg()
	rospy.Subscriber('/control_robot_yaska1',Int32, ControlCallback)
	# Pour ploter la trajectoire du robot
	rospy.Subscriber('/yaska1/sequence_move_group/result', MoveGroupSequenceActionResult, TrajectoryResultCallback)
	...
	clientSequence_ = actionlib.SimpleActionClient('/yaska1/sequence_move_group', MoveGroupSequenceAction)
	...
	planingScene = moveit_commander.PlanningSceneInterface(ns= "/yaska1")
	#Création de la navette
	rospy.loginfo("Import de la navette ...")
	name = "Navette"
	scale = (0.1,0.1,0.1)
	filename = "src/Yaskawa_hc10/yaskawa1/meshes/env/Navette.stl"
